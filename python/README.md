...
### 5. `run_sweep.py`
*   **Purpose**: Volume Clock Sensitivity Analysis.
*   **Workflow**:
    1.  Runs the C++ engine multiple times with different `--volume-bucket` values (e.g., 0.5, 1.0, 2.0 BTC).
    2.  Parses the output to extract Sharpe Ratios.
    3.  Identifies the optimal volume threshold.

### 6. `adverse_selection_analysis.py`
*   **Purpose**: Post-Trade Analysis.
*   **Workflow**:
    1.  Reads the trade logs generated by the engine (`--trade-logs`).
    2.  Calculates post-fill returns (mark-to-market) at various horizons (1s, 5s, 10s).
    3.  Correlates returns with the alpha signal to validate predictive power.
    4.  Generates `adverse_selection_report.png`.
